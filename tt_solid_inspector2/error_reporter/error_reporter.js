...
var _wine_er_callback_queue = [];

function _er_flush_queue() {
  var el = document.getElementById("WINE_BRIDGE");
  if (el) {
    el.value = JSON.stringify(_wine_er_callback_queue);
  }
}

function _WineAckCallbacks() {
  _wine_er_callback_queue = [];
  _er_flush_queue();
}
  ...
  return {
    ...
    callback : function(event_name, data) {
      ...
      setTimeout(function() {
        ...
        // Queue for Wine polling fallback.
        _wine_er_callback_queue.push(event_name + "|" + json);
        _er_flush_queue();
        // Also try skp: protocol (native Windows).
        try {
...
var Bridge = function() {
  return {
    ...
    init : function() {
      ...
      $bridge.hide();
      $("body").append($bridge);
      var $wine = $("<input type='hidden' id='WINE_BRIDGE' value='[]' />");
      $("body").append($wine);
    },

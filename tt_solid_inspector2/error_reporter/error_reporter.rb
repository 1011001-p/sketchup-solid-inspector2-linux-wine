  ...
  class ErrorReporter
    ...
    def create_dialog(extension_id, extension)
      ...
      window.add_action_callback("callback") { |dialog, event_name|
        # If skp: works, stop polling.
        if @er_poll_timer
          UI.stop_timer(@er_poll_timer)
          @er_poll_timer = nil
        end
        dialog.execute_script("_WineAckCallbacks();") rescue nil
      ...
      # Wine polling fallback: read WINE_BRIDGE hidden input.
      @er_poll_timer = UI.start_timer(0.2, true) {
        if window.visible?
          begin
            json = window.get_element_value("WINE_BRIDGE")
            if json && json.size > 0 && json != "[]"
              messages = JSON.parse(json)
              if messages.is_a?(Array) && messages.size > 0
                window.execute_script("_WineAckCallbacks();")
                messages.each do |msg|
                  parts = msg.to_s.split("|", 2)
                  evt = parts[0].to_s
                  data_str = parts[1].to_s
                  data = (data_str.size > 0) ? JSON.parse(data_str) : nil
